...
- **Fix A (scelta utente a)**: `_GATE_TOP_CEILING_ONLY` default → **false**. `_coherence_ok` ora accetta se plausibile a un ceiling AFFIDABILE (5500 **O** 5000), non solo il top. Le vocali posteriori arrotondate (/ɔː/) mistrackano legittimamente a 5500 → non vanno più rifiutate. Il tradeoff sabotaggio è coperto dal gate Whisper. Verificato: presa "Law." (5000: 389/1069) → accettata; sabotaggio che fitta solo a 4500 → ancora rifiutato.
- **⚠️ SCOPE V2 (LIMITE DI FONDO — documentato, non un bug):** il motore a **finestra singola** misura la **QUALITÀ** della vocale target, NON la **TRAIETTORIA** formantica. Quindi V2 valuta se una /ɔː/ è ben fatta, ma **non separa nettamente le minimal pair vocaliche** (law /ɔː/ vs low /əʊ/ danno entrambi ~35, perché su "low" la finestra stabile prende l'onset simil-/ɔː/). **Cosa promette V2**: (a) Whisper sgama i sabotaggi grossi (parola chiaramente diversa/italiana) → A1; (b) le formanti valutano la qualità della vocale target. **Cosa NON fa V2**: la distinzione chirurgica tra vocali simili (monottongo↔dittongo, minimal pair) → è **v3/Charsiu (forced alignment + traiettoria)**. Non fingiamo di farla col motore attuale. Decisione utente: NON ottimizzare la variabilità F1 sulle posteriori (limatura del motore a finestra singola) — si risolve con v3.
- **⚠️ RETEST UTENTE**: "law" ben detto ora deve dare un **voto formantico REALE** (non A1-secco né rifiuto), coerente con la qualità della pronuncia; palestra/gym → A1; CAT → risultato resta visibile.
...
- **FIX 21/07 (3ª review) — 422 mai acquisito + DIAGNOSI punto 3**:
  - Punto 2 RISOLTO: `MockRecorder` ora ha `onError` (chiamato su 422/errore) → lo step isolato **invalida** il fonema (rimuove anche un punteggio valido precedente). Avanzamento bloccato finché non ci sono 3 punteggi validi. Un take rifiutato non entra mai nel verdetto.
  - **DIAGNOSI punto 3 (root cause del caso FOOT-detto-TRAP)**: il retry a tetti LPC decrescenti (5500→5000→4500) in `_measure_all_ceilings`/`_select_measurement` accetta misure **spurie** al tetto basso. Verificato: /iː/ reale valutato come /ʊ/ → a 5500/5000 F2≈2400-2524 (giustamente implausibile), ma a **4500 Hz** il tracker tira F2 a **1360** → cade nel range /ʊ/ → HTTP 200 con score A2 invece di 422. → una vocale sbagliata spesso NON dà 422 ma un punteggio basso su finestra mistracked.
  - **IN INDAGINE 21/07 (4ª review)** → **RISOLTO (5ª review, su log voce reale)**:
    - **Fix DSP browser (AGC/NS/EC off)** applicato a `MockRecorder` (level-test) E `StudentRecordingStudio` (Fase-2): confermato sui log reali che F1 non è più sistematicamente basso (es. /ʊ/ corretto F1=388 vs rif 379). Curva D/pesi restano validi (bias era costante).
    - **Fix gate — regola del TETTO PIÙ ALTO**: il gate `_coherence_ok` ora richiede plausibilità al **tetto tested più alto** (5500 Hz), non "a un tetto qualsiasi di 5500/5000". Motivo (da log voce reale): il sabotaggio FOOT era implausibile a 5500 (F1=775) e diventava plausibile solo scendendo a 5000 (F1=574) → mistracking. Le pronunce corrette sono sempre plausibili a 5500. Tunable via env `LEVEL_TEST_GATE_TOP_CEILING_ONLY` (default true). Il gate NON era bypassato: la pipeline reale lo attraversa (loggato), la soglia era solo troppo larga.
    - **LIMITE INTRINSECO documentato per v2**: il sabotaggio TRAP ("palestra" detto per /æ/) produce F1=535/F2=1828 → plausibile a 5500 per /æ/ → accettato legittimamente come /æ/ di bassa qualità. L'analisi formantica misura la QUALITÀ VOCALICA, non l'identità della parola: non può distinguere "cat" da un /a/ italiano vicino. Serve riconoscimento del fono (Charsiu/ASR) → **v2**. Mitigazione UX v1 (fatta): step isolato mostra la PAROLA grande + IPA + come pronunciarla (`lt-isolated-prompt`), così l'utente reale dice "cat", non borbotta. TODO v1: aggiungere audio di riferimento del prof da imitare PRIMA di registrare (in M2.3 audio pre-gen).
