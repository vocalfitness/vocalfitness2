...
- [x] **Onboarding Wizard multi-step** (07/02/2026, iter. 11-12) — 5 step modale per la prenotazione "Individual diagnostic assessment" + Lead Inbox CRM admin. Renderizzato via `createPortal` su `document.body` (z-index 9999) per evitare problemi di posizionamento dentro Navbar fixed. Trigger: hero CTA, final CTA, navbar CTA, **Sticky CTA bottom-left**. Bilingue IT/EN, ESC + backdrop close, validazione per step, persistenza backend con campi strutturati `role`/`nativeLanguage`/`motivation`/`source`. Backward-compatible.
- [x] **Admin Lead Inbox** (07/02/2026, iter. 12) — Nuovo tab "Lead Inbox" in AdminPage con: badge sorgente (Wizard blu / Form grigio), badge CEFR colorato per livello (A1-A2 slate, B1-B2 blue, C1-C2 emerald), filtri rapidi (search testo, source, CEFR, role, sector, native language), drawer di dettaglio laterale con tutti i campi + cambio status (pending/contacted/qualified/closed) + email diretta. Endpoint: `GET /api/admin/leads` (filtri query string), `PATCH /api/admin/leads/{id}` (status/notes).
