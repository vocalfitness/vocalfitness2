...
- **FIX 21/07 (3ª review) — 422 mai acquisito + DIAGNOSI punto 3**:
  ...
  - **IN INDAGINE 21/07 (4ª review) — discrepanza curl↔voce reale + F1 sistematicamente basso**: utente riferisce che dal vivo (a) TRAP-detto-FOOT passa il gate e (b) il fonema viene segnato "acquisito" (422 non scatta), MA via curl gate+422 funzionano. Indizio: verdetto dice "F1 troppo basso" su TUTTI e 3 i fonemi → sospetto misura F1 sistematicamente bassa nel contesto live. DIFFERENZE trovate level-test vs studio Fase-2: (1) studio invia `dialect`+`target_kind`+`reference_url`, level-test solo `file`+`phoneme_ipa` (scora entrambi i dialetti, best by composite); (2) studio usa `pickMimeType()`, recorder level-test usava default; (3) entrambi usavano `getUserMedia({audio:true})` → DSP browser (AGC/NS/EC) attivo che distorce F1. AZIONI: disattivato DSP browser nel recorder level-test (`echoCancellation/noiseSuppression/autoGainControl:false`); aggiunto **logging diagnostico** (`DIAG`) in `/level-test/score` (F1/F2/F3, f0, gruppo, riferimenti, ceiling, attempts, coherence). NB fixture pulita /iː/ misura F1=237 < ref (RP 280 / AmE 342). ATTESA: test voce reale utente sui 3 fonemi per leggere i DIAG log e distinguere misura-frontend vs selezione-riferimento. NON risolto — non dichiarare risolto senza dati reali.
