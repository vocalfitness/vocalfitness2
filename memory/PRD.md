...
- **FIX 21/07 (3ª review) — 422 mai acquisito + DIAGNOSI punto 3**:
  ...
  - **PUNTO 3 RISOLTO — opzione (b), vowel-coherence gate SOLO in `/level-test/score`** (motore Fase-2 condiviso NON toccato → zero rischio regressione produzione). Principio: il Level Test è un ESAME (l'utente può produrre il suono sbagliato → rigore), Fase-2 è PRATICA (retry generoso). Il gate (`_coherence_ok`) accetta il punteggio di un dialetto solo se il target è plausibile ad **almeno un tetto affidabile** (`_RELIABLE_CEILINGS`, default 5500/5000, **tarabile** via env `LEVEL_TEST_RELIABLE_CEILINGS_HZ`); se plausibile solo a 4500 → mistracking → 422. Messaggio **azionabile** per fonema (`_EXAMPLE_WORD`): es. "Non ho riconosciuto chiaramente il suono [ʊ]. Riprova pronunciando la vocale di 'foot'." Verificato via curl: /iː/→/iː/ 200 B1; /iː/→/ʊ/ 422; /iː/→/æ/ 422. Frontend mostra il messaggio e invalida il fonema (`onError`).
