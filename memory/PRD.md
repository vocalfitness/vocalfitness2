...
- **FIX 21/07 (3ª review) — 422 mai acquisito + DIAGNOSI punto 3**:
  ...
  - **DIAGNOSI punto 3 (root cause del caso FOOT-detto-TRAP)**: il retry a tetti LPC decrescenti (5500→5000→4500) in `_measure_all_ceilings`/`_select_measurement` accetta misure **spurie** al tetto basso. Verificato: /iː/ reale valutato come /ʊ/ → a 5500/5000 F2≈2400-2524 (giustamente implausibile), ma a **4500 Hz** il tracker tira F2 a **1360** → cade nel range /ʊ/ → HTTP 200 con score A2 (49.8) invece di 422. → una vocale sbagliata spesso NON dà 422 ma un punteggio basso su finestra mistracked. Il fix del punto 2 è necessario ma NON sufficiente per il wrong-vowel: serve chiudere il punto 3 (selezione tetto/finestra più rigorosa). DA DECIDERE con l'utente prima del punto M2.2.
