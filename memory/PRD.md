...
- [x] **Homepage redesign istituzionale B2B/scientifico** (07/02/2026, iter. 10) — Palette unificata WHITE + BLUE medical/scientific su TUTTO il sito. 13 sezioni in EN/IT bilingue + Footer light + 4 video con shimmer loader. Sezioni: Hero (light), Credibility Strip, **Trusted by Industry & Academia** (light + LOGO MARQUEE infinito Stripe-style), What VocalFitness Is, How The Method Works (banner methodVideo), Who It Is For (4 card light bianche), Designed to integrate (blue accent + integrationVideo), Programmes (SpeakRight 101), CEFR & Outcomes, Excellence Testimonials (light), Selected Contexts, **Steve Dapper (LIGHT, white cards + dapperVideo)**, dual-path Final CTA. **Footer convertito a light theme** (white/blue, slate-200 borders, blue-600 accents). 5 video corporate con `<VideoWithLoader>` (shimmer + spinner blu + label "Loading video", IntersectionObserver lazy-load rootMargin 200px, preload='metadata' per startup veloce). Modali e StickyCTA i18n EN/IT. Navbar a 7 voci.
