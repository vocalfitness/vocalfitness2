...
# Map of known proposal pages to the public PDF that should be attached.
# Keep here (close to the endpoint) so new proposals can be added in one place.
PROPOSAL_PDFS = {
    "proposta-ey": {
...
class ProposalSendRequest(BaseModel):
    ...
    page: str
    ref: Optional[str] = None
    notify_seller: bool = True  # CC steve@vocalfitness.org

@api_router.post("/proposals/send-by-email")
async def send_proposal_by_email(payload: ProposalSendRequest, request: Request):
    ...
    SMTP failure is reported back as `email_sent: false` without leaking the
    ...
    try:
        ...
        print(f"[send_proposal] PDF download failed: {e}")
        raise HTTPException(status_code=502, detail="Impossibile scaricare il PDF della proposta in questo momento")
    ...
    if smtp_password and smtp_server and smtp_user:
        try:
            with smtplib.SMTP(smtp_server, smtp_port, timeout=20) as srv:
                ...
                rcpts = [email_to]
                if payload.notify_seller and seller_email and seller_email.lower() != email_to.lower():
                    rcpts.append(seller_email)
                srv.sendmail(msg['From'], rcpts, msg.as_string())
                email_sent = True
        except Exception as e:
            smtp_error = str(e)
            print(f"[send_proposal] SMTP send failed: {e}")
    else:
        print("[send_proposal] SMTP not configured — email NOT sent")
    ...
    try:
        ...
        print(f"[send_proposal] audit log insert failed: {e}")

    if not email_sent:
        # We never expose SMTP details to the client.
        raise HTTPException(status_code=503, detail="Servizio email temporaneamente non disponibile, riprova più tardi")
...
@api_router.get("/admin/elevenlabs/voices")
async def list_elevenlabs_voices(current_user: dict = Depends(get_current_user)):
    ...
    if not client:
        raise HTTPException(status_code=503, detail="ElevenLabs non configurato (ELEVENLABS_API_KEY mancante)")
    try:
...
@api_router.post("/admin/elevenlabs/tts")
async def elevenlabs_tts(req: ElevenLabsTTSRequest, current_user: dict = Depends(get_current_user)):
    ...
    if not voice_id:
        raise HTTPException(status_code=400, detail="voice_id richiesto (oppure setta ELEVENLABS_DEFAULT_VOICE_ID)")
    ...
    try:
        ...
        if fmt.startswith("mp3"):
            ...
            ext = "pcm"; content_type = "audio/L16"
        elif fmt.startswith("ulaw"):
            ext = "ulaw"; content_type = "audio/basic"
        else:
            ext = "mp3"; content_type = "audio/mpeg"
